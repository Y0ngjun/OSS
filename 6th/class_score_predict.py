...
if __name__ == '__main__':
    ...
    # Load score data
    class_kr = np.loadtxt('6th/data/class_score_kr.csv', delimiter=',')
    class_en = np.loadtxt('6th/data/class_score_en.csv', delimiter=',')
    data = np.vstack((class_kr, class_en))

    # Estimate a line, final = slope * midterm + y_intercept
    A = np.vstack((data[:,0], np.ones(data[:,0].shape))).T
    b = data[:,1]
